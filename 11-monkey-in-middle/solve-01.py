...
for r in range(MAX_ROUNDS):
    for monkey in all_monkeys:
        ...
        while len(monkey[KEY_ITEMS]):
            ...
            new_worry = monkey[KEY_OPERATION](item, monkey[KEY_OPERATION_ARGUMENT])
            monkey[KEY_INSPECTION_COUNT] += 1
            # There's relief that the inspection didn't damage the item
            new_worry = new_worry // 3
            # print(f"Monkey gets bored so worry is divided by three to {new_worry}")
